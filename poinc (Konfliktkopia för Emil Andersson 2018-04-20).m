function AF_cand_Poincare = poinc(RRinterval, window_size, step_size)
    ...
    while p <= (length(RRinterval)-window_size/2) %p=position of calculation (mitten av foenstret)
        ...
        if d <= 0.06 %0.06
            ...
            AF_cand=1;
            %if systematically(RR_test,11) == 1  %returns yes for clusters or no for 1 or more than 10 clusters %testing for clusters up to 11 
                %systematically --> clustering algorithm based on k-mean [9] in [50]
                %AF_cand=0;
                %c 
